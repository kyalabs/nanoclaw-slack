...
import { createTask, getAllTasks, getDueTasks, getTaskById, logTaskRun, updateTask, updateTaskAfterRun, } from './db.js';
                ...
                if (fs.existsSync(archivePath)) {
                    try {
                        ...
                        if (archived.status === 'complete') {
                            fs.unlinkSync(filePath);
                            logger.info({ file, groupDir }, 'Removed stale state file (archived as complete)');
                        }
                    }
    ...
    try {
        const output = await runContainerAgent(group, {
            ...
            if (terminalOutput && effectiveResult) {
                ...
                if (task.silent) {
                    logger.debug({ taskId: task.id }, 'Silent task — result captured in logs, not forwarded to chat');
                }
                else {
                ...
                logger.debug({ taskId: task.id, eventType: streamedOutput.event_type }, 'Ignoring non-terminal scheduled task output');
            }
    ...
    if (task.source_group) {
        ...
        try {
            ...
            const prFiles = fs.readdirSync(ipcStateDir)
                .filter((f) => f.startsWith('git-pr-') && f.endsWith('.json'))
                .sort()
                .reverse();
        ...
        const groups = deps.registeredGroups();
        const sourceEntry = Object.entries(groups).find(([, g]) => g.folder === task.source_group);
        if (sourceEntry) {
            ...
            logger.info({ taskId: task.id, callbackId, sourceGroup: task.source_group }, 'Chain callback task created');
        }
        else {
            logger.warn({ taskId: task.id, sourceGroup: task.source_group }, 'Source group not found for chain callback');
        }
    }
}
