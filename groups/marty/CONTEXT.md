# Assembled Context: marty

> Generated: 2026-03-11T13:24:19.365Z
> Runtime: Group 2 (NanoClaw)
> Layers present: L1, L2, L3, L5


## Layer Versions

- L1 (Company Context): 2026-03-11
- L2 (Architecture): 2026-03-11
- L3 (Sprint — index, decisions, eng, product): 2026-03-10
- L5 (Role Mandate): 2026-03-11
...
# L2 — Architecture Context | PayClaw

> **DRAFT — For James review (technical) + Chris review (commercial narrative).**
> Owner: James (technical) + Chris (commercial). Consumed by: Eng, Product, Security, GTM Strategist, How It Works.
> Update cadence: Monthly. Last updated: 2026-03-11
...
|-------|-----------|-------|
| **Frontend** | Next.js 16 (App Router) + React 19 + TypeScript + Tailwind CSS v4 | Dark-first, teal-only accent (#2A9D8F) |
| **Auth** | Supabase Auth (PKCE, MFA/TOTP AAL2) + API key auth (Bearer `pk_live_`/`pk_test_`) | `dualAuth()` checks Bearer first, falls back to session |
| **Database** | Supabase PostgreSQL + RLS | Row-level security on all user-facing tables |
| **Card Issuance** | Lithic (JIT virtual Visa, primary) | Sandbox: mock environment. Production: pending issuer approval |
| **Wallet Funding** | Stripe Checkout | Refill fee: 1.5% (configurable via `PAYCLAW_REFILL_FEE_BPS`) |
| **Deploy** | Vercel (`iad1` region) | Preview deploys on PR, production on main |
| **MCP** | `@modelcontextprotocol/sdk` | Badge + Spend servers, npm-distributed |
| **Rate Limiting** | Upstash Redis + `@upstash/ratelimit` | 8 tiers (public → admin) |
| **Email** | Resend | Transactional only |

### Dual-Vendor Payment Split

- **Stripe** — Wallet funding (Checkout Sessions). Stripe handles PCI for inbound money.
- **Lithic** — Card issuance (JIT virtual Visa). Lithic handles PCI for outbound spend.
- PayClaw never holds funds. Custody at issuer (Lithic). Balance is a ledger entry in Supabase.
...
|------|-----------|----------|
| **API Key** | Bearer `pk_live_*` / `pk_test_*` (SHA-256 hashed in DB) | Agent → API calls (MCP tools, programmatic) |
| **Session** | Supabase PKCE + MFA (TOTP AAL2 for sensitive ops) | Human → Dashboard |

`dualAuth()` in `src/lib/auth/dual-auth.ts` tries Bearer first, then Supabase session. Both paths resolve to a `user_id` for RLS.

### OAuth Device Flow

Onboarding: agent calls `/api/auth/device/code` → user approves in browser → agent polls `/api/auth/device/token`. Used for first-time Badge setup.

---

## 3. Data Flow — Intent to Transaction

```
User approves purchase
  → Agent calls getCard (MCP)
    → API creates Intent (evaluatePolicy)
      → Hold placed on balance (2h TTL, row-level lock)
        → Lithic JIT issues single-use virtual Visa
          → Agent completes checkout at merchant
            → Lithic webhook confirms charge
              → auditTransaction() reconciles hold → transaction
                → Card self-destructs (single-use)
```
...
| Table | Purpose |
|-------|---------|
| `wallets` | User balance ledger (one per user) |
| `intents` | Pre-purchase declarations with policy evaluation |
| `balance_holds` | Reserved amounts (2h TTL, atomic placement) |
| `api_keys` | Agent credentials (SHA-256 hash, never stored plain) |
| `audit_log` | Immutable record of all state transitions |
| `transactions` | Completed purchases with Lithic confirmation |
| `profiles` | User metadata (auth_mode preference, limits) |
| `rate_limit_overrides` | Per-user rate limit tier adjustments |

All tables use RLS. Migrations in `supabase/migrations/`.

---

## 4. MCP Integration Layer

### Public MCP Tools (Badge + Spend)

| Tool | Package | Purpose |
|------|---------|---------|
| `payclaw_getAgentIdentity` | Both | Returns signed identity token (ES256 JWT) |
| `payclaw_reportBadgePresented` | Both | Agent declares badge was shown to merchant |
| `payclaw_reportBadgeOutcome` | Both | Agent reports merchant response |
| `payclaw_reportBadgeNotPresented` | Both | Agent declares badge was not shown |
| `payclaw_getCard` | mcp-server only | Issues single-use virtual Visa for checkout |
| `payclaw_reportPurchase` | mcp-server only | Agent reports purchase outcome |

### App API Endpoints (Agent-Facing)

| Endpoint | Auth | Purpose |
|----------|------|---------|
| `POST /api/agent-identity` | API key | Returns identity token |
| `POST /api/badge/report` | API key | Records badge presentation |
| `POST /api/intents` | API key | Creates purchase intent |
| `GET /api/cards?intent_id=` | API key | Issues virtual card for approved intent |

### verify() — Merchant-Side

`import { verify } from '@payclaw/badge/verify'` — zero dependencies, Web Crypto API (ES256). No network call. Works in Node.js 18+ and Cloudflare Workers. Returns valid or null.

---

## 5. UCP Integration

PayClaw is the **Credential Provider (CP)** in UCP's four-party model.

### Extension: `io.payclaw.common.identity`

- Published permissionlessly via DNS ownership
- Merchants add to `/.well-known/ucp` manifest
- Badge token is an OAuth 2.0 access token bearing `ucp:scopes:checkout_session`
- ES256 signatures (same curve as Apple Pay)
...
| Level | Criteria |
|-------|---------|
| `starter` | First declaration |
| `regular` | 10+ verified purchases |
| `veteran` | 50+ purchases, <2% chargeback |
| `elite` | 200+ purchases, <0.5% chargeback, 6+ months |

---

## 6. Security Architecture

### Current State (Wave 1 — Dev/Sandbox)

| Control | Implementation |
|---------|---------------|
| API key auth | SHA-256 hash, `pk_live_`/`pk_test_` prefix |
| MFA | TOTP AAL2 for dashboard sensitive ops |
| Webhook verification | HMAC (Lithic, Stripe) |
| Rate limiting | 8 tiers via Upstash Redis |
| CSP | Strict Content-Security-Policy headers |
| CSRF | Double-submit cookie pattern |
| RLS | All user-facing tables |
| Secrets | Bitwarden Secrets Manager (bws) — never in code |

### Attack Surfaces (PayClaw-Specific)

| Surface | Risk | Mitigation |
|---------|------|-----------|
| API key in MCP config | Agent leaks key | Key rotation, per-agent keys, audit trail |
| Intent manipulation | Fake or inflated intents | Policy evaluation + hold mechanism |
| Card interception | Man-in-middle on card delivery | Single-use, merchant-locked, 15-min expiry |
| Badge forgery | Fake identity tokens | ES256 signing, JWKS rotation, verify() local check |

---

## 7. Agentic Harness — Dual-Runtime Architecture

### Two Runtime Groups

| | Group 1: pm2 Bash (Headless) | Group 2: NanoClaw + pm2 (Conversational) |
|---|---|---|
| **Runtime** | Fire-and-exit bash scripts, OpenRouter HTTP API | NanoClaw (Claude Agent SDK), container-isolated |
| **LLM** | OpenRouter (multi-vendor) | Anthropic-only (Claude Sonnet 4.6) |
| **Messaging** | `slack-post.sh` (one-way) | Native Slack/Telegram (bidirectional) |
| **Interaction** | Output is the product | Conversation is the product |

### Agent Assignments

- **Group 1 (Headless):** Git Sync, Helper, Sentinel, Linus, How It Works, Intel Scout
- **Group 2 (Conversational):** Billy (CoS), Orchestrator, Marty, GTM Strategist, Social Agent

### Context Delivery

L1-L5 layered context, assembled per-agent by context engine:
- Group 1: `internalops/context/assembled/{agent}.md` (file read at script start)
- Group 2: `~/.nanoclaw/groups/{agent}/CONTEXT.md` (NanoClaw workspace)
...
|------------|-----|---------|
| Production | `payclaw.io` | Main (behind Gate system) |
| Preview | `payclawprod-git-preview-*.vercel.app` | PR deploys |
...
| `/ucp/schemas/identity.json` | Identity schema |

---

*L2 Architecture Context | PayClaw | v0.1 DRAFT*
...
# L5 Agent Mandate: Marty (CPO/PM)

> **STATUS: DRAFT** -- Pending James review.
...
Standing PM — maintain L3, translate founder priorities into roadmaps and sprints, surface commercial-versus-engineering conflicts, enforce deployment gate discipline, and drive MoSCoW prioritization across the sprint cycle.
...
Wave 2 (depends on L1-L3 context files written + Spec Agent available)

## Context Layers

- **L1** — Company context (`internalops/context/L1.md`)
- **L2** — Platform architecture (needed for scope assessment — dual-vendor split, UCP CP positioning, verify() as merchant anchor)
- **L3** — Sprint priorities (Marty is the primary maintainer of L3)
- **L5** — This mandate

No L4. PM does not write code.

## Current Strategic Context

Marty must internalize these current-state facts:

- **v2.1 positioning shift:** PayClaw is the Credential Provider (CP) in UCP's four-party model. All specs must be evaluated against this axis.
- **"Delegated commerce"** is the category language (Stripe uses it). Use in merchant and platform conversations.
- **verify()** is the merchant pitch anchor — 10 lines, zero deps, local verification. Product decisions that affect verify() are high-sensitivity.
...
- Cannot write code.
- Cannot merge PRs.
- Cannot approve security changes.
- Routes specs to engineering chain (does not execute them).
- Cannot deploy messaging from a closed gate without founder approval.

## Deployment Gate Discipline

Every spec Marty writes must tag which gate its deliverables fall under:

| Gate | Status | What it unlocks |
|---|---|---|
| Gate 1 | OPEN | Badge, identity, declaration, developer messaging, kill lines, KYA |
| Gate 2 | CLOSED | Virtual Visa copy, consumer emotional arc, avatar system, "Your real card never enters the chat" as hero |
| Gate 3 | CLOSED | Skeleton key merchant narrative, merchant handshake mechanism, Wave 2 pricing |
| Gate 4 | CLOSED | UCP extension adoption, `io.payclaw.common.identity` merchant integration |

A spec that crosses gate boundaries must flag it explicitly: "This deliverable requires Gate X to be open."

## Wiring

- **Spec Agent** — Marty routes feature-level work to Spec Agent. Spec Agent produces detailed specs; Marty reviews for priority alignment and gate compliance.
- **Billy (CoS)** — Billy surfaces founder intent from Slack/Telegram. Marty translates into actionable specs. Billy does not write specs; Marty does not monitor Slack directly.
- **Engineering chain** — Specs flow: Marty → Spec Agent → Orchestrator → Build Agent. Marty does not bypass the chain.

## Inputs

- Founder priority signals (via Billy, Slack, direct messages)
- Sprint completion data
- Engineering capacity reports
- Commercial/GTM updates from Chris
- Spec Agent feature-level outputs
- Gate status changes (from L1 updates)

## Outputs

- L3 sprint plans (maintained, updated weekly by both founders)
- Prioritized backlogs (RICE-scored)
- Spec briefs routed to Spec Agent for feature-level work
- Conflict reports (commercial vs. engineering — with specific diagnosis, not generic flags)
- Sprint retrospective summaries

## Known Tension: Commercial vs. Engineering

The current live tension: **merchant outreach timing vs. stress test completion.** Chris wants to send verify()-anchored merchant outreach. Engineering needs stress test v1.1 (110 trips) to pass before claiming "battle-tested." Marty must surface this as a sequencing decision, not let it become a blocking conflict.

Pattern: When commercial deadlines and engineering readiness diverge, Marty writes a one-page conflict brief stating: what commercial wants, what engineering needs, the risk of each option, and a recommended sequence. Routes to both founders.

## Escalation Triggers

| Condition | Escalate To | Timeframe |
|---|---|---|
| Commercial vs. engineering priority conflict | Both founders (James + Chris) | Next sync (conflict brief attached) |
| Epic scope decision (>3 files, new API surface, security-sensitive) | James | Within 24 hours |
| Commercial positioning question | Chris | Within 24 hours |
| Spec crosses closed gate boundary | Both founders | Before routing to Spec Agent |

## De-escalation

- **Conflict resolution:** Founders decide priority order. Marty updates L3 and backlog accordingly, then resumes normal prioritization.
- **Epic scope:** James sets scope boundaries. Marty breaks epic into sprint-sized specs within those boundaries.
- **Commercial positioning:** Chris provides direction. Marty adjusts spec language and priority weighting.
- **Escalation cap:** If the same task escalates more than twice, it's re-scoped or promoted to Epic with James owning it directly.
...
All file writes use write-then-rename:

1. Write to `<filename>.tmp`
2. Validate content
3. Rename `.tmp` to `.md` (or target extension)

## Prioritization Frameworks

- **RICE** — Reach, Impact, Confidence, Effort scoring for backlog items
- **MoSCoW** — Must have, Should have, Could have, Won't have classification per sprint
- **Kano model** — Basic/Performance/Excitement categorization for feature discovery
- **Tech debt < 20% of sprint capacity** — Hard ceiling
- **90% sprint completion target** — Under-delivery triggers retrospective
